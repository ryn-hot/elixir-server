...
mod tests {
    ...
    fn multi_target_request() -> AnimeMatchRequest {
        let mut request = request();
        request.target.scope = AnimeMatchScope::Range;
        request.target.wanted_target_keys = vec!["S02E01".to_string(), "S02E02".to_string()];
        request.target.episode_numbers = vec![1, 2];
        request.target.absolute_episode_numbers = vec![13, 14];
        ...
        request.candidates[0].files.push(AnimeMatchFile {
            file_key: "candidate-0-file-1".to_string(),
            path: "Tokyo Ghoul Root A - 02.mkv".to_string(),
        });
        request.candidates[0].parse_facts = AnimeMatchParseFacts {
            title_candidates: vec!["Tokyo Ghoul Root A".to_string()],
            episode_numbers: vec![1, 2],
            batch_kind: Some("pack".to_string()),
            audio_profiles: vec!["dual_audio".to_string()],
            languages: vec!["ja".to_string(), "en".to_string()],
            ..Default::default()
        };
        request.candidates.push(AnimeMatchCandidate {
            candidate_key: "candidate-1".to_string(),
            title: "Tokyo Ghoul Root A Episodes 1-2".to_string(),
            files: Vec::new(),
